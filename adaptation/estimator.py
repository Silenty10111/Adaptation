...
4. GaitAdaptationController — 集成控制器，对接 adaptation.gait 输出
...
class EKFComOffsetEstimator:
    ...
    def __init__(
        ...
        gait_plan: Dict,
        description: Dict,
        process_noise: float = 2e-4,  # ↑ 1e-4→2e-4: faster adaptation to morphology changes
        obs_noise: float = 8e-3,       # ↑ 5e-3→8e-3: more tolerance to simulation noise
    ):
        """
        Parameters
        ----------
        gait_plan      : adaptation.gait 输出（用于提取足端几何）
    ...
    def _build_observation_matrix(
        ...
        偏航动力学简化模型：
            ω_yaw ≈ (1/Izz) * Σ_i (F_i × r_i) · k
        其中 F_i ∝ swing_amplitude_i * fwd_axis，r_i = foot_i − com

        质心偏移 Δc 导致每条腿的力臂变化：
            r_i_actual = foot_i − (com_nominal + Δc)
            cross(r_i_actual, fwd) ≈ cross(r_i_nominal, fwd) + cross(−Δc, fwd)
                                   = ψ_i_nominal + (Δcx * fwd_y − Δcy * fwd_x)

        因此观测量 Δω = ω_measured − ω_nominal 与 Δc 的关系为：
            Δω ≈ (1/Izz) * Σ_i [swing_i * (fwd_y * Δcx − fwd_x * Δcy)] + ω_bias
        """
        fwd = np.asarray(self._plan.get("final_forward_axis", [1., 0.]), dtype=float)
        fwd_n = float(np.linalg.norm(fwd))
        if fwd_n > 1e-9:
        ...
        # Sum swing contributions to compute effective lever from CoM offset
        topo = self._plan.get("topology", {})
        amps = topo.get("per_leg_stride_amplitudes", {})
        sum_swing = 0.0
        for lid in active_leg_ids:
            amp = float(amps.get(str(lid), 0.5))
            sum_swing += amp

        n_active = max(len(active_leg_ids), 1)
        avg_swing = sum_swing / n_active

        # Physical coupling: Δω = (avg_swing/Izz) * [fwd_y, -fwd_x] · Δc + ω_bias
        Izz = max(total_mass * 0.06, 1e-3)  # estimated yaw inertia (increased for stability)

        # H = [α * fwd_y,  -α * fwd_x,  1.0]
        # where α = avg_swing / Izz (thrust-to-yaw coupling)
        alpha = avg_swing / Izz
        H = np.array([
            [alpha * fwd[1], -alpha * fwd[0], 1.0]
        ], dtype=float)
        return H
    ...
    def update(
        ...
        # 协方差更新 (Joseph form for numerical adaptation.stability)
...
class GaitAdaptationController:
    ...
    def __init__(
        ...
        gait_plan: Dict,
        # 增益参数
        com_offset_gain: float = 2.2,    # ↑ 1.5→2.2: stronger CoM offset correction
        yaw_rate_gain: float = 1.2,      # ↑ 0.8→1.2: stronger yaw rate correction
        friction_freq_gain: float = 0.4, # 摩擦降低 → 频率修正增益
        max_amp_adj: float = 0.45,       # ↑ 0.35→0.45: allow larger amplitude adjustments
        ...
        # 平滑因子（EMA） — ↑ 0.15→0.25: faster response to yaw correction
    ...
    def compute_corrections(
        ...
        for lid, base_amp in self._base_amps.items():
            ...
            target_amps[lid] = float(np.clip(base_amp + delta, 0.20, 0.92))
...
class OnlineStateEstimator:
    ...
    3. 将 adaptation 中的修正量应用到步态控制：
        per_leg_amps = state.adaptive_stride_scales
        freq_scale = state.frequency_scale
