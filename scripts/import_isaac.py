...
def estimate_drop_torque_margin(description: Dict[str, object], gait_plan: Dict[str, object]) -> Dict[str, float]:
    ...
    # Use adaptation.stability module when available; fall back to plan values otherwise.
    if _HAS_STABILITY:
    ...
    if not _HAS_STABILITY:
        print("[WARN] adaptation.stability.py not found; SSM will report 0.0.")
    ...
    try:
        ...
        if args.use_mpc:
            try:
                ...
                raise RuntimeError(
                    "启用 --use-mpc 需要 adaptation.mpc.py 和 srb_mpc_demo.py 以及 scipy。"
                ) from exc
