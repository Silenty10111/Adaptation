...
class VirtualLegController:
    ...
    def generate_alternating_groups(self) -> Tuple[List[int], List[int], List[int]]:
        """
        基于拓扑矩阵自动生成腿组分配，替代 adaptive_gait 的启发式分组。
...
def zero_shot_gait_plan(
    ...
    与 adaptive_gait.compute_adaptive_plan() 的区别
    ------------------------------------------------
    · 不依赖启发式腿组分组（极坐标轮流分配）
    · 通过拓扑不变性矩阵自动推导最优腿组
    ...
    Returns
    -------
    Dict  兼容 adaptive_gait 输出格式的步态计划
    """
    # 步骤 1：从描述中提取足端位置和质心
    foot_positions_xy: Dict[int, np.ndarray] = {}
    ...
    # 步骤 6：构造 per_leg_stride_amplitudes（与 adaptive_gait 格式兼容）
    ...
    # 步骤 8：组装步态计划（兼容 adaptive_gait 输出格式）
