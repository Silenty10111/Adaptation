...
4. integrate_dynamic_symmetry() — 一键接口，与 adaptive_gait 输出对接
...
class DynamicSymmetryGaitPlan:
    """
    动态对称性步态计划。
    扩展了 adaptive_gait 的基础计划，添加非对称参数。
    """
    base_plan: Dict                          # adaptive_gait 原始输出
...
def build_initial_leg_params(
    ...
    """
    从 adaptive_gait 输出构造初始 LegGaitParams。
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gait-plan", type=Path, default=None,
                        help="adaptive_gait 输出 JSON；不传则自动计算")
