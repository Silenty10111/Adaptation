...
3. YawTorqueBalancer   — 轻量级包装器，调用 GRFAllocator 只求解 XY 平面偏航力矩，
   ...
   adaptive_gait.compute_adaptive_plan() 的输出格式。

使用示例
--------
>>> from centroidal_wbc import YawTorqueBalancer
...
class YawTorqueBalancer:
    ...
    ``adaptive_gait.compute_adaptive_plan()`` 的输出。
    ...
    与原始 adaptive_gait 中 YAW_COMP_GAIN=0.5 不同，
...
def run_centroidal_wbc(
    ...
    可直接替换 adaptive_gait.compute_adaptive_plan() 中的
    YAW_COMP_GAIN 启发式逻辑。
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gait-plan", type=Path, default=None,
                        help="adaptive_gait 输出的 JSON；不传则自动计算")
